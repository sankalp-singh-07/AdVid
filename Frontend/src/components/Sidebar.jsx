        ...
        <>
            ...
            `}>
                <div className="flex items-center gap-3 px-6 py-6 border-b border-[#E8EAF5]">
                    ...
                    <span className="text-xl font-extrabold text-slate-850 tracking-tight">EnterpriseAI</span>
                </div>
                ...
                <div className="flex-1 overflow-y-auto py-4 px-4 space-y-1">
                    ...
                    {navItems.map((item) => (
                        ...
                        >
                            {({ isActive }) => (
                                <>
                                    <item.icon size={20} className={`shrink-0 transition-colors duration-200 ${isActive ? "text-white" : "text-slate-400 group-hover:text-[#6D5DFC]"}`} />
                                    <span>{item.name}</span>
                                </>
                            )}
                        </NavLink>
                    ...
                    {isLoggedIn ? (
                        <div className="flex flex-col gap-3">
                            <div className="px-3 py-2 bg-[#FAFBFF] border border-[#E8EAF5] rounded-lg text-sm text-slate-500">
                                Logged in as <span className="text-slate-850 font-bold block truncate">{user?.email || 'User'}</span>
                            </div>
                            <button
                                onClick={logout}
